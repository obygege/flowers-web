...
app.post('/api/orders', async (req, res) => {
  const {
    product_id, delivery_type, delivery_address, google_maps_link, delivery_date,
    time_slot, sender_name, sender_phone, card_from, card_to, card_message, voucher_code
  } = req.body;
  if (!product_id || !delivery_type || !delivery_date || !time_slot || !sender_name || !sender_phone) {
  ...
  try {
    await connection.beginTransaction();
    const [products] = await connection.query('SELECT id, name, price FROM products WHERE id = ? AND is_active = TRUE', [product_id]);
    if (!products.length) {
    ...
    if (voucher_code) {
      const [vouchers] = await connection.query(`SELECT v.* FROM vouchers v
        LEFT JOIN voucher_products vp ON vp.voucher_id = v.id AND vp.product_id = ?
        WHERE v.code = ? AND v.starts_at <= NOW() AND v.expires_at > NOW()
        AND (NOT EXISTS (SELECT 1 FROM voucher_products WHERE voucher_id = v.id) OR vp.product_id IS NOT NULL)`, [product_id, String(voucher_code).trim().toUpperCase()]);
      if (!vouchers.length) {
        await connection.rollback();
        return res.status(400).json({ success: false, message: 'Voucher tidak ditemukan atau sudah kedaluwarsa' });
      }
      validVoucher = vouchers[0];
      if (Number(products[0].price) < Number(validVoucher.min_order)) {
        await connection.rollback();
        return res.status(400).json({ success: false, message: `Minimal order voucher ${Number(validVoucher.min_order).toLocaleString('id-ID')}` });
      }
      discount = validVoucher.discount_type === 'percent'
        ? Math.min(Number(products[0].price), Number(products[0].price) * Number(validVoucher.discount_value) / 100)
        : Math.min(Number(products[0].price), Number(validVoucher.discount_value));
    }
    const totalAmount = Number(products[0].price) - discount;
...
app.get('/api/admin/dashboard', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const [orders] = await db.query(`
      SELECT o.id, o.order_number, o.total_amount, o.status, o.delivery_type, o.delivery_address,
        o.delivery_date, o.time_slot, o.sender_name, o.sender_phone, o.card_from, o.card_to, o.card_message,
        o.created_at, p.name AS product_name, p.description AS product_description,
        p.price AS product_price, c.name AS category_name, COALESCE(pi.image_url, ?) AS product_image,
        oi.quantity
      FROM orders o
